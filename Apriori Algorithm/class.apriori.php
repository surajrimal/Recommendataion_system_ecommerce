<?php 
/**
 * Apriori Algorithm: Suraj Rimal
 * The default minimun confidence is 75% and support is 2 
 */
class Apriori {
    private $delimiter   = ','; 
    private $minSup      = 2; 
    ...
    /**
        1. The table makes items
        2. Each item is produced to access key
        3. All items are calculated and repeat them - Level 1
        Note: The calculation is repeated at least
    **/
    ...
    { 
       ...
       if(!is_array($db))
          {
             $db = file($db);  //read entire file into an array
          }
  
       $num = count($db);  
       for($i=0; $i<$num; $i++) 
          {
             $tmp  = explode($this->delimiter, $db[$i]); //explode — Split a string by string
             $num1 = count($tmp);
             $x    = array();
             for($j=0; $j<$num1; $j++) 
                {
                   $x = trim($tmp[$j]); //trim — Strip whitespace (or other characters) from the beginning and end of a string
                   if($x==='')
                      {
                         continue;
                      }
             ...
             if($sup>=$this->minSup)
                {
                   ...
                   $tmp[] = array($item);//filter items by minSup
                }
          }
    ...
    /**
        1. Svprymvm value with respect to input the ID number of the items
    **/
    ...
    /**
        1.Combine two arrays and remove excess
    **/
    ...
    /**
        1.According to the item name item ID or item on returns
           {1,2,3,4} => {A,B,C,D}
    **/
    ...
    /**
        1.Item sets duplicates returns
    **/
    ...
    { 
       ...
       //A subset of a larger collection eliminates the
    ...
    /**
        1.The final rules regarding the calculation of the minimum Kanfynds
    **/
